"""
tests/test_api.py
-----------------
Pytest test suite for the AI-IDS Flask API.
COM668 | Abdulbosit Abdurazzakov | B00979380

Tests cover:
  - /health         -- liveness and model metadata
  - /features       -- feature list shape and content
  - /predict        -- valid input, named fields, missing features, bad values
  - /predict/batch  -- valid batch, oversized batch, malformed input
  - /api/feed       -- returns list structure
  - /api/stats      -- returns aggregate fields
  - /api/shap       -- returns feature importance list
  - /dashboard      -- HTML page served correctly
  - Error handlers  -- 404 returns JSON

CI strategy
-----------
The trained .pkl artefacts are not committed to the repo (they are large
binary files). When running in CI, the fixture detects their absence and
patches ids.models.predict.load_models with lightweight stubs so all API
contract tests can run without the real models.

Run locally (with real models):
  pytest tests/ -v

Run in CI (stubs injected automatically):
  pytest tests/ -v
"""

import os
import sys
from unittest.mock import MagicMock, patch

import numpy as np
import pytest

# ── Make the src/ package importable from the project root ────────────────────
_ROOT = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
_SRC  = os.path.join(_ROOT, "src")
for _p in [_ROOT, _SRC]:
    if _p not in sys.path:
        sys.path.insert(0, _p)

_MODELS_PRESENT = (
    os.path.exists(os.path.join(_ROOT, "data", "processed", "feature_names.pkl"))
    and os.path.exists(os.path.join(_ROOT, "models", "random_forest.pkl"))
)

# ── Stub helpers (used when real models are absent) ───────────────────────────

_STUB_FEATURES = [f"feature_{i}" for i in range(78)]
# Include the real name so test_destination_port_present passes in CI too
_STUB_FEATURES[0] = "Destination Port"


def _make_stub_rf():
    """Minimal RandomForestClassifier stub."""
    rf = MagicMock()
    rf.n_estimators = 150
    rf.predict.return_value = np.array([0])           # ATTACK
    rf.predict_proba.return_value = np.array([[0.9, 0.1]])
    rf.feature_importances_ = np.ones(78) / 78        # uniform importance
    return rf


def _make_stub_iso():
    """Minimal IsolationForest stub."""
    iso = MagicMock()
    iso.contamination = 0.1
    iso.predict.return_value = np.array([-1])          # anomaly
    iso.decision_function.return_value = np.array([-0.05])
    return iso


def _make_stub_scaler():
    scaler = MagicMock()
    scaler.transform.side_effect = lambda X: X        # identity transform
    return scaler


def _make_stub_le():
    """LabelEncoder stub: classes_ = ['ATTACK', 'BENIGN']."""
    le = MagicMock()
    le.classes_ = np.array(["ATTACK", "BENIGN"])
    le.inverse_transform.side_effect = lambda x: np.array(
        ["ATTACK" if v == 0 else "BENIGN" for v in x]
    )
    return le


# ── Fixtures ──────────────────────────────────────────────────────────────────

@pytest.fixture(scope="session")
def feature_names():
    """Return feature names — real ones if available, stubs otherwise."""
    if _MODELS_PRESENT:
        import joblib
        return joblib.load(os.path.join(_ROOT, "data", "processed", "feature_names.pkl"))
    return list(_STUB_FEATURES)


@pytest.fixture(scope="session")
def client(feature_names):
    """
    Create a Flask test client.

    When real model artefacts are present, loads them normally.
    In CI (artefacts absent), patches load_models with lightweight stubs
    so all API contract tests run without the real trained models.
    """
    if _MODELS_PRESENT:
        import src.api as api_module
        api_module.app.config["TESTING"] = True
        with api_module.app.test_client() as c:
            yield c
    else:
        stub_scaler  = _make_stub_scaler()
        stub_le      = _make_stub_le()
        stub_rf      = _make_stub_rf()
        stub_iso     = _make_stub_iso()

        with patch("ids.models.predict.load_models",
                   return_value=(stub_scaler, stub_le, feature_names,
                                 stub_rf, stub_iso)):
            import src.api as api_module
            # Patch module-level globals (already bound at import time)
            api_module.scaler        = stub_scaler
            api_module.le            = stub_le
            api_module.feature_names = feature_names
            api_module.rf_model      = stub_rf
            api_module.iso_model     = stub_iso
            api_module.app.config["TESTING"] = True
            with api_module.app.test_client() as c:
                yield c


@pytest.fixture(scope="session")
def valid_vector(feature_names):
    """A zeroed feature vector (valid shape, all zeros)."""
    return [0.0] * len(feature_names)


# ── /health ───────────────────────────────────────────────────────────────────

class TestHealth:
    def test_status_200(self, client):
        assert client.get("/health").status_code == 200

    def test_returns_healthy(self, client):
        assert client.get("/health").get_json()["status"] == "healthy"

    def test_models_listed(self, client):
        data = client.get("/health").get_json()
        assert "random_forest" in data["models"]
        assert "isolation_forest" in data["models"]

    def test_feature_count_positive(self, client):
        assert client.get("/health").get_json()["features"] > 0


# ── /features ─────────────────────────────────────────────────────────────────

class TestFeatures:
    def test_status_200(self, client):
        assert client.get("/features").status_code == 200

    def test_count_matches_list(self, client):
        data = client.get("/features").get_json()
        assert data["count"] == len(data["features"])

    def test_features_non_empty(self, client):
        assert len(client.get("/features").get_json()["features"]) > 0

    def test_destination_port_present(self, client):
        assert "Destination Port" in client.get("/features").get_json()["features"]


# ── /predict ──────────────────────────────────────────────────────────────────

class TestPredict:
    def test_valid_array_input(self, client, valid_vector):
        r = client.post("/predict", json={"features": valid_vector})
        assert r.status_code == 200
        data = r.get_json()
        assert data["status"] == "ok"
        assert data["result"]["final_decision"] in ("ATTACK", "BENIGN")

    def test_valid_named_input(self, client, feature_names):
        payload = {name: 0.0 for name in feature_names}
        assert client.post("/predict", json=payload).status_code == 200

    def test_result_contains_all_models(self, client, valid_vector):
        result = client.post("/predict", json={"features": valid_vector}).get_json()["result"]
        assert "random_forest" in result
        assert "isolation_forest" in result
        assert "ensemble" in result

    def test_rf_confidence_in_range(self, client, valid_vector):
        rf = client.post("/predict", json={"features": valid_vector}).get_json()["result"]["random_forest"]
        assert 0.0 <= rf["confidence"] <= 1.0
        assert 0.0 <= rf["attack_prob"] <= 1.0
        assert 0.0 <= rf["benign_prob"] <= 1.0

    def test_inference_time_reported(self, client, valid_vector):
        data = client.post("/predict", json={"features": valid_vector}).get_json()
        assert data["inference_time_ms"] >= 0

    def test_wrong_feature_count_returns_400(self, client):
        r = client.post("/predict", json={"features": [0.0, 1.0, 2.0]})
        assert r.status_code == 400
        assert "Expected" in r.get_json()["error"]

    def test_missing_named_feature_returns_400(self, client, feature_names):
        payload = {name: 0.0 for name in feature_names[1:]}
        r = client.post("/predict", json=payload)
        assert r.status_code == 400
        assert "Missing" in r.get_json()["error"]

    def test_non_numeric_feature_returns_400(self, client, feature_names):
        payload = {"features": ["not_a_number"] + [0.0] * (len(feature_names) - 1)}
        assert client.post("/predict", json=payload).status_code == 400

    def test_nan_feature_returns_400(self, client, feature_names):
        bad = [0.0] * len(feature_names)
        bad[0] = "NaN_string_value"
        assert client.post("/predict", json={"features": bad}).status_code == 400

    def test_empty_body_returns_400(self, client):
        r = client.post("/predict", data="", content_type="application/json")
        assert r.status_code == 400

    def test_get_not_allowed(self, client):
        assert client.get("/predict").status_code == 405


# ── /predict/batch ────────────────────────────────────────────────────────────

class TestPredictBatch:
    def test_single_flow_batch(self, client, valid_vector):
        r = client.post("/predict/batch", json={"flows": [valid_vector]})
        assert r.status_code == 200
        assert r.get_json()["count"] == 1

    def test_multiple_flows(self, client, valid_vector):
        r = client.post("/predict/batch", json={"flows": [valid_vector] * 5})
        assert r.get_json()["count"] == 5

    def test_oversized_batch_returns_400(self, client, valid_vector):
        r = client.post("/predict/batch", json={"flows": [valid_vector] * 1001})
        assert r.status_code == 400
        assert "1000" in r.get_json()["error"]

    def test_missing_flows_key_returns_400(self, client):
        assert client.post("/predict/batch", json={"data": []}).status_code == 400

    def test_wrong_length_flow_gets_error_entry(self, client, valid_vector):
        results = client.post(
            "/predict/batch", json={"flows": [valid_vector, [0.0, 1.0]]}
        ).get_json()["results"]
        assert "result" in results[0]
        assert "error" in results[1]


# ── Dashboard data endpoints ──────────────────────────────────────────────────

class TestDashboardAPI:
    def test_feed_returns_list(self, client):
        r = client.get("/api/feed")
        assert r.status_code == 200
        assert isinstance(r.get_json()["feed"], list)

    def test_stats_returns_expected_fields(self, client):
        data = client.get("/api/stats").get_json()
        for field in ("total", "attacks", "benign", "attack_pct",
                      "avg_confidence", "avg_latency_ms"):
            assert field in data

    def test_shap_returns_features(self, client):
        data = client.get("/api/shap?top_n=5").get_json()
        assert len(data["features"]) == 5
        assert "feature" in data["features"][0]

    def test_feed_populated_after_predict(self, client, valid_vector):
        client.post("/predict", json={"features": valid_vector})
        data = client.get("/api/feed?n=1").get_json()
        assert data["count"] >= 1
        assert "final_decision" in data["feed"][0]


# ── Dashboard HTML ────────────────────────────────────────────────────────────

class TestDashboard:
    def test_dashboard_returns_200(self, client):
        assert client.get("/dashboard").status_code == 200

    def test_dashboard_is_html(self, client):
        assert b"<html" in client.get("/dashboard").data

    def test_dashboard_contains_chart_js(self, client):
        assert b"chart.umd.min.js" in client.get("/dashboard").data


# ── Error handlers ────────────────────────────────────────────────────────────

class TestErrorHandlers:
    def test_404_returns_json(self, client):
        r = client.get("/nonexistent-endpoint-xyz")
        assert r.status_code == 404
        assert r.get_json()["error"] == "Endpoint not found."

    def test_405_returns_json(self, client):
        assert "error" in client.get("/predict").get_json()
